...
class RadarSimulator:
    ...
    def _simulate_single_radar(self, radar_model: RadarModel, 
        ...
        print(radar.radar_prop['transmitter'])
        print(radar.radar_prop['receiver'])
        print(radar.sample_prop)
        ...
        if len(targets) > 0:  # 防止没有目标时浪费资源  
            ...
            for t in [0]:
                ...
                print(f"Simulating radar {radar_id} at time {timestamp:.2f}s")
                ...
                print(f"Processed range profile shape: {processed_data['range_profile'].shape}")
                print(f"Processed doppler profile shape: {processed_data['doppler_profile'].shape}")
                print(f"Processed rd_map shape: {processed_data['rd_map'].shape}")
                print(f"Processed detection_map shape: {processed_data['detection_map'].shape}")
                # 目标检测
    ...
    def _range_processing(self, baseband_data: np.ndarray, 
        ...
        return np.mean(np.abs(range_profiles), axis=1)  # 平均距离像
    ...
    def _cfar_detection(self, processed_data: Dict[str, Any],
        ...
        """恒虚警检测"""
        # 简化CFAR实现
        if processed_data['doppler_profile'] is not None:
            data = np.abs(processed_data['doppler_profile'])
        else:
    ...
    def _detect_targets(self, processed_data: Dict[str, Any],
        ...
        try:
            ...
            exec_str = traceback.format_exc()
            self.logger.error(f"目标检测失败: {exec_str}")
