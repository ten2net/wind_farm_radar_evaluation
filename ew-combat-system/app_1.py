...
st.markdown("""
    **专业的电子战体系对抗仿真与评估平台（简化版）**
    
    支持一对一、多对一、多对多对抗想定，提供完整的电磁环境构建、对抗仿真、效能评估和可视化功能。
    
    *注：此版本使用Matplotlib进行可视化，确保稳定运行。*
""")
...
def show_results_analysis():
    ...
    with export_col3:
        if st.button("保存所有结果", use_container_width=True, type="primary"):
            try:
                all_data = {
...
def show_performance_assessment():
    ...
    try:
        ...
        if metrics:
            ...
            ax.set_title('性能雷达图', size=16, y=1.1)
            ax.grid(True)
...
def show_cleanup_maintenance():
    ...
    with col1:
        ...
        if st.button("清理旧文件", type="secondary", use_container_width=True):
            try:
    ...
    with backup_col2:
        ...
        if st.button("恢复备份", use_container_width=True, type="secondary"):
            try:
...
st.markdown(
    ...
    <div style="text-align: center; color: gray;">
        <p>电子战对抗仿真系统 v1.0.0 (简化版) | © 2024 电子战仿真实验室</p>
        <p>技术支持: support@ew-simulation.com | 使用Matplotlib可视化引擎</p>
    </div>
    """,
