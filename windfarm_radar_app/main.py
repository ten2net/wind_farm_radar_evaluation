...
try:
    ...
    from utils.yaml_loader import load_scenario_yaml
...
def show_home_page():
    ...
    with col1:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">
                ...
                {len(st.session_state.current_scenario['wind_turbines']) if st.session_state.scenario_loaded else 0}
            </div>
        </div>
        """, unsafe_allow_html=True)
    
    with col2:
        radar_count = len(st.session_state.current_scenario['radar_stations']) if st.session_state.scenario_loaded else 0
        st.markdown(f"""
        <div class="metric-card">
    ...
    with col3:
        comm_count = len(st.session_state.current_scenario['communication_stations']) if st.session_state.scenario_loaded else 0
        st.markdown(f"""
        <div class="metric-card">
    ...
    with col4:
        target_count = len(st.session_state.current_scenario['targets']) if st.session_state.scenario_loaded else 0
        st.markdown(f"""
        <div class="metric-card">
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        if st.session_state.scenario_loaded:
            scenario_name = st.session_state.current_scenario.get('name', '未命名场景')
