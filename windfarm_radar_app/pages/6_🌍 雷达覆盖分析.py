...
def create_sector_polygon(center_lat, center_lon, bearing, beam_width, range_km, num_points=20):
    ...
    for i in range(num_points + 1):
        current_bearing = start_bearing + \
            (end_bearing - start_bearing) * (i / num_points)
        arc_lat, arc_lon = calculate_destination(
            center_lat, center_lon, current_bearing, range_km)
...
def generate_fixed_targets(radar_lat, radar_lon, coverage_data, targets_config, num_targets=20, seed=42):
    ...
    for i in range(num_targets):
        ...
        # 计算目标位置
        target_lat, target_lon = calculate_destination(
            radar_lat, radar_lon, angle, distance)
    ...
    if num_turbines > 0:
        if layout_type == 'grid':
            ...
            for i in range(num_turbines):
                ...
                turbine_lon = radar_lon + x / \
                    (111000.0 * math.cos(math.radians(radar_lat)))
                distance_km = calculate_distance(
                    radar_lat, radar_lon, turbine_lat, turbine_lon)
            ...
            for i in range(num_turbines):
                ...
                turbine_lat = radar_lat +0.08 + y / 111000.0
                turbine_lon = radar_lon + x / \
                    (111000.0 * math.cos(math.radians(radar_lat)))
                distance_km = calculate_distance(
                    radar_lat, radar_lon, turbine_lat, turbine_lon)
...
with col_map:
    ...
    if radar_config and 'position' in radar_config:
        ...
        radar_lat = 39.1 + radar_y / 111000.0
        radar_lon = 119.1 + radar_x / (111000.0 * math.cos(math.radians(40.0)))
    else:
        radar_lat = 39.1
        radar_lon = 119.1
        radar_z = 50.0
    ...
    if wind_farm_config and not coverage_data.get('turbines_generated', False):
        turbines = generate_fixed_turbines(
            radar_lat, radar_lon, wind_farm_config)
    ...
    if targets_config and not coverage_data.get('targets_generated', False):
        ...
        targets = generate_fixed_targets(
            radar_lat, radar_lon, coverage_data, targets_config, num_targets)
...
宽度: {beam_width:.1f}°''',
            ...
            fill=True,
            fill_color='rgba(255, 150, 255, 0.2)',
            fill_opacity=0.8,
            weight=2
        ).add_to(m)
...
with col_control:
    ...
    with tab1:
        ...
        with col_lat:
            radar_lat_input = st.number_input(
                ...
                min_value=-90.0,
                max_value=90.0,
                value=float(coverage_data.get('radar_lat', 39.0)),
                step=0.001,
                format="%.6f",
                key="ctrl_radar_lat"
            )
        ...
        with col_lon:
            radar_lon_input = st.number_input(
                ...
                min_value=-180.0,
                max_value=180.0,
                value=float(coverage_data.get('radar_lon', 119.0)),
                step=0.001,
                format="%.6f",
                key="ctrl_radar_lon"
            )
    ...
    with tab2:
        ...
        with col_radar2:
            ...
            st.metric(
                "覆盖半径", f"{coverage_data.get('coverage_range', 0):.0f}km")
        ...
        if targets:
            ...
            targets_in_range = sum(
                1 for t in targets if t.get('in_range', False))
            avg_detection_prob = np.mean([t.get('detection_prob', 0) for t in targets if t.get(
                'in_range', False)]) if targets_in_range > 0 else 0
        ...
        if turbines:
            ...
            if total_turbines > 0:
                avg_distance = np.mean(
                    [t.get('distance_km', 0) for t in turbines])
    ...
    with tab3:
        ...
        has_radar = 'radar_config' in st.session_state and bool(
            st.session_state.radar_config)
        has_wind_farm = 'wind_farm_config' in st.session_state and bool(
            st.session_state.wind_farm_config)
        has_targets = 'targets_config' in st.session_state and len(
            st.session_state.get('targets_config', [])) > 0
